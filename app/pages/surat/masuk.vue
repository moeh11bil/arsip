...
useHead({ title: 'Surat Masuk - EduArch' })
...
const search = ref('')
const type = ref<SuratType>('MASUK')
  ...
  <div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      <div>
        ...
        <p class="text-sm text-gray-500 mt-0.5">Kelola dan tindak lanjut surat masuk sekolah</p>
      </div>
        ...
        >
          ...
          <div class="flex-1 min-w-0">
            ...
            <div class="flex items-center gap-3 mt-1 text-xs text-gray-500">
              ...
              <span v-if="surat.pengirim" class="hidden sm:inline">·</span>
              <span v-if="surat.pengirim" class="hidden sm:inline truncate max-w-xs">{{ surat.pengirim }}</span>
            </div>
          </div>
    ...
    <div v-else class="bg-white/80 backdrop-blur-sm rounded-2xl border border-white/60 shadow-sm p-12 text-center">
      <Icon name="heroicons:envelope" class="w-16 h-16 mx-auto text-gray-300" />
      <h3 class="mt-4 text-lg font-semibold text-gray-900">Belum ada surat</h3>
      <p class="mt-1 text-sm text-gray-500">Mulai dengan membuat surat pertama Anda.</p>
    ...
    <!-- Create Modal -->
    <UiSuratModal v-model="showCreateModal" :type="type" @created="refresh()" />
  </div>
</template>
