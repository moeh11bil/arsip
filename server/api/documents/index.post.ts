...
export default defineEventHandler(async (event) => {
  try {
    ...
    if (!user) {
      throw createError({ statusCode: 401, statusMessage: 'User tidak terautentikasi' })
    }
    ...
    const uploadsData = await Promise.all(
      fileItems.map(async (fi) => {
        ...
        return {
          ...
          mimeType: fi.type || 'application/octet-stream',
    ...
    throw createError({ statusCode: 500, statusMessage: 'Gagal membuat dokumen: ' + (error.message || 'Unknown error') })
  }
})
